...
namespace ApiJobMarket.Controllers
{
    ...
    public class ApiQualificationController : BaseApiController
    {
        private readonly ILog logger = LogProvider.For<ApiQualificationController>();

        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> GetList()
        {
            var requestName = "Qualifications-GetList";
            ...
            try
            {
                ...
                var listQualification = CommonHelpers.GetListQualifications();

                await Task.FromResult(listQualification);                

                returnModel.value = listQualification;
        ...
        public async Task<IHttpActionResult> GetDetail(int id)
        {
            ...
            var requestName = "Qualifications-GetDetail";
            var returnModel = new ApiResponseCommonModel();
            var jsonString = string.Empty;
            ...
            try
            {
                ...
                var returnCode = EnumCommonCode.Success;
                var storeQualification = GlobalContainer.IocContainer.Resolve<IStoreQualification>();

                var info = storeQualification.GetById(id);
