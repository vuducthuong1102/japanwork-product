...
{
    [Authorize]
    [RoutePrefix("api/sub_fields")]
    public class ApiSubFieldController : BaseApiController
    {
        private readonly ILog logger = LogProvider.For<ApiSubFieldController>();

        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> GetList()
        {
            var requestName = "SubFields-GetList";
            ...
            try
            {
                ...
                var listSubField = CommonHelpers.GetListSubFields();

                await Task.FromResult(listSubField);                

                returnModel.value = listSubField;
