...
{
    [Authorize]
    [RoutePrefix("api/japanese_levels")]
    public class ApiJapaneseLevelController : BaseApiController
    {
        private readonly ILog logger = LogProvider.For<ApiJapaneseLevelController>();

        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> GetList()
        {
            var requestName = "JapaneseLevels-GetList";
            ...
            try
            {
                ...
                var listJapaneseLevel = CommonHelpers.GetListJapaneseLevels();

                await Task.FromResult(listJapaneseLevel);                

                returnModel.value = listJapaneseLevel;
        ...
        public async Task<IHttpActionResult> GetDetail(int id)
        {
            ...
            var requestName = "JapaneseLevels-GetDetail";
            var returnModel = new ApiResponseCommonModel();
            var jsonString = string.Empty;
            ...
            try
            {
                ...
                var returnCode = EnumCommonCode.Success;
                var storeJapaneseLevel = GlobalContainer.IocContainer.Resolve<IStoreJapaneseLevel>();

                var info = storeJapaneseLevel.GetById(id);
