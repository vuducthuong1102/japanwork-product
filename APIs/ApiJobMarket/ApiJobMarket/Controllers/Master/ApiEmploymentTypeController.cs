...
{
    [Authorize]
    [RoutePrefix("api/employment_types")]
    public class ApiEmploymentTypeController : BaseApiController
    {
        private readonly ILog logger = LogProvider.For<ApiEmploymentTypeController>();

        [HttpGet]
        [Route("")]
        public async Task<IHttpActionResult> GetList()
        {
            var requestName = "EmploymentTypes-GetList";
            ...
            try
            {
                ...
                var listEmploymentType = CommonHelpers.GetListEmploymentTypes();

                await Task.FromResult(listEmploymentType);                

                returnModel.value = listEmploymentType;
        ...
        [HttpGet]
        [Route("{id:int}/fields")]
        public async Task<IHttpActionResult> GetListFields(int id)
        {
            CreateDocumentApi(id);
            var requestName = "EmploymentTypes-GetListFields";
            var returnModel = new ApiResponseCommonModel();
            var jsonString = string.Empty;
            ...
            try
            {
                ...
                var returnCode = EnumCommonCode.Success;
                var storeField = GlobalContainer.IocContainer.Resolve<IStoreField>();

                var info = storeField.GetListFieldsByEmploymentType(id);
        ...
        {
            CreateDocumentApi(id);
            var requestName = "EmploymentTypes-GetDetail";
            var returnModel = new ApiResponseCommonModel();
            var jsonString = string.Empty;
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                ...
                var returnCode = EnumCommonCode.Success;
                var storeEmploymentType = GlobalContainer.IocContainer.Resolve<IStoreEmploymentType>();

                var info = storeEmploymentType.GetById(id);
                await Task.FromResult(info);
