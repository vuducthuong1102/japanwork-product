...
namespace ApiJobMarket.HttpTracking
{
    ...
    public sealed class HttpTrackingMiddleware : OwinMiddleware
    {
        ...
        public override async Task Invoke(IOwinContext context)
        {
            ...
            {
                Application = ApiJobMarketSettings.MDS_FromAppName,
            ...
            context.Response.OnSendingHeaders(state =>
            {
                ...
                resp.Headers.Add(trackingIdPropertyName_, new[] { record.TrackingId, });
            ...
            record.RequestBody = await WriteContentAsync(requestStream, record.RequestHeaders, maxRequestLength_);

            WriteResponseHeaders(response, record);
            record.ResponseLength = responseStream.ContentLength;
            record.ResponseBody = await WriteContentAsync(responseStream, record.ResponseHeaders, maxResponseLength_);

            
            //sw.Stop();
            record.CallBackTime = DateTime.Now;            
            record.CallDuration = (record.CallBackTime - record.CallDateTime);
            //record.CallDuration = TimeSpan.FromMilliseconds(sw.ElapsedMilliseconds);
        ...
        {
            record.RequestVerb = request.Method;
            record.RequestUri = request.Uri;
            record.RequestHeaders = request.Headers;
            record.RequestIpAddress = request.RemoteIpAddress;
        }
