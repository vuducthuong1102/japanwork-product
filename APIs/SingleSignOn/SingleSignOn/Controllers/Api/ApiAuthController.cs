...
namespace SingleSignOn.Controllers
{
    ...
    public class ApiAuthController : BaseApiController
    {
                ...
                if (isValid)
                {
                    ...
                    if (cachedData == null)
                    {
                        ...
                        if (apiLoginIdentity != null)
                        {
                            ...
                            if (apiLoginIdentity.EmailConfirmed == 0)
                            {
        ...
        [Route("resend_email_active")]
        [HttpPost]
        public async Task<IHttpActionResult> ResendEmailActiveAccount(ApiSendEmailActiveAccountModel model)
        {
            ...
            try
            {
                ...
                if (isValid)
                {
                    ...
                    var userInfo = userStore.GetUserByUserName(model.Email);

                    if (userInfo == null)
                    {
                        returnModel.Msg = UserApiResource.COMMON_ERROR_ACCOUNT_BY_EMAIL_NOT_FOUND;
                    }
                    else
                    {
                        if (userInfo.EmailConfirmed)
                        {
                            returnModel.Msg = UserApiResource.LB_THIS_ACCOUNT_ALREADY_ACTIVED;
                        }
                    }
                    ...
                    var hashingData = GenereateHashingForNewAccount(userInfo.UserName, userInfo.UserName);

                    var sendMailStatus = SendEmailToActiveAccount_WithoutAmazon(userInfo.UserName, hashingData);

                    await Task.FromResult(sendMailStatus);

                    returnModel.Code = sendMailStatus;
                    if (sendMailStatus == EnumCommonCode.Success)
                    {
                        userStore.ResendEmailActive(new IdentityActiveAccount
                        {
                            UserName = model.Email,
                            HashingData = hashingData
                        });

                        returnModel.Msg = string.Format(EmailResource.NOTIF_EMAIL_SENT_FORMAT, userInfo.UserName);
                    }
                    else
                    {
        ...
        [Route("send_email_recover")]
        [HttpPost]
        public async Task<IHttpActionResult> RecoverPassword(ApiSendEmailRecoverPasswordModel model)
        {
            ...
            try
            {
                //logger.Debug("Begin SendEmailRecoverPassword-API request");
                ...
                if (isValid)
                {
                    ...
                    {
                        if (userInfo.LockoutEnabled && userInfo.LockoutEndDate != null)
                        {
                            returnModel.Msg = UserApiResource.COMMON_ERROR_BLOCKED_ACCOUNT;
                        }
                    }
                    ...
                    var sendMailStatus = BeginSendEmailToRecoverPwd(userInfo.Id, userInfo.UserName, PasswordLevelType.Level1);
        ...
        public async Task<IHttpActionResult> ActiveAccount(ApiActiveAccountModel model)
        {
            ...
            try
            {
                ...
                if (isValid)
                {
                    ...
                    var userStore = GlobalContainer.IocContainer.Resolve<IStoreUser>();
                    var userInfo = userStore.GetUserByUserName(info.UserName);
        ...
        public async Task<IHttpActionResult> RecoverPasswordStep1(ApiChangePassword1Model model)
        {
            ...
            try
            {
                ...
                //SendEmail to user
                var sendMailStatus = BeginSendEmailToRecoverPwd(identity.Id, identity.Email, model.PasswordType);
                returnModel.Code = sendMailStatus;
                if (sendMailStatus == EnumCommonCode.Success)
                {
