...
{    
    public class WebAuthController : BaseController
    {
        ...
        public async Task<ActionResult> RecoverPassword(string token)
        {
            ...
            WebRecoverPasswordModel model = new WebRecoverPasswordModel();
            ...
            try
            {
                ...
                var rawData = Utility.DecryptText(dataArr[1], SystemSettings.EncryptKey);
                await Task.FromResult(rawData);

                var userData = rawData.Split('|');
                if (userData.Count() >= 2)
                {                    
                    model.UserId = Utils.ConvertToInt32(userData[0]);
                    model.HashingData = token;

                    var userIdentity = _userStore.GetUserById(model.UserId);
                    if (userIdentity != null)
                    {
                        if (userIdentity.SecurityStamp != model.HashingData)
                        {
                            this.AddNotification(UserWebResource.COMMON_ERROR_DATA_INVALID, NotificationType.ERROR);
        ...
        public ActionResult RecoverPassword_Post(WebRecoverPasswordModel model)
        {
            ...
            try
            {
                ...
                userIdentity = _userStore.GetUserById(model.UserId);
                if (userIdentity != null)
                {
                    if (userIdentity.SecurityStamp != model.HashingData)
                    {
                    ...
                    userIdentity.PasswordHash = Utility.Md5HashingData(model.NewPassword);

                    _userStore.RecoverPassword(userIdentity, model.HashingData);
