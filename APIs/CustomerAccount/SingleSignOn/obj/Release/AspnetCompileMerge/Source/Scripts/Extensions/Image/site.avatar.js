...
var maxSizeAllowed = 3;     // Upload limit in MB
...
function initAvatarUpload() {
    $('#avatar-crop-box').html($('#avatar-crop-box-temp').html());
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.success === false) {
                ...
                var img = $('#crop-avatar-target');
                img.attr('src', data.fileName);
                img.attr('data-filename', data.baseFileName);
                $("#TempUploadAvatar").val(data.baseFileName);
...
function saveAvatar() {
    ...
    $.ajax({
        ...
        data: {
            ...
            fileName: $("#TempUploadAvatar").val()
        }
    }).done(function (data) {
        if (data.success === true) {
            $('#avatar-result img').attr('src', data.avatarFileLocation);
            $("#Avatar").val(data.newFileName);
