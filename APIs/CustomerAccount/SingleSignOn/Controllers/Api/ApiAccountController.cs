...
namespace SingleSignOn.Controllers
{
    ...
    public class ApiAccountController : BaseApiController
    {
                ...
                if (isValid)
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        if (isPhoneNumber)
                        {
                            ...
                            var tranId = CreateNewOTPForAction(result, string.Empty, otpCode, string.Empty, ActionType.ActiveAccount, ref createOTPStatus);

                            if (createOTPStatus == EnumCommonCode.Success)
                            {
                                if (!string.IsNullOrEmpty(tranId))
                                {
                                    SendOTPCodeToUser(model.UserName, otpCode, tranId, userIdentity.OTPType);
                                }

                                returnModel.Msg = string.Format(UserApiResource.COMMON_SUCCESS_OTPCODE_SENT, model.UserName);
                                returnModel.Code = EnumCommonCode.Success;
                            }
                            else
                            {
                    ...
                    {
                        returnModel.Msg = GetRegisterResponseMessage(returnModel.Code);
                    }
                }
        ...
        private int SendEmailToActiveAccount(string email, string hashingData, int userId = 0)
        {
            var result = EnumCommonCode.Success;
            try
            {
        ...
        private int SendEmailToActiveAccount_WithoutAmazon(string email, string hashingData)
        {
            var result = EnumCommonCode.Success;
            //Begin sending email
