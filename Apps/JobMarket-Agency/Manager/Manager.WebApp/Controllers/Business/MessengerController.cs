...
namespace Manager.WebApp.Controllers
{
    public class MessengerController : BaseAuthedController
    {
        ...
        public ActionResult GetSupporters()
        {
            return PartialView("Partials/_Supporters");
        }
        ...
        public async Task<ActionResult> GetOperatorsList(CommonPagingModel model)
        {
            ...
            try
            {
                ...
                var apiReturn = AdminServices.GetListOperatorsForMessengerAsync(apiModel).Result;
        ...
        public async Task<ActionResult> OpenConversation()
        {
            ...
            try
            {
                ...
                var targetUserResult = AdminServices.GetOperatorProfileAsync(new ApiCommonOperatorInfoModel { Id = receiverId }).Result;
            ...
            {
                var strError = string.Format("Failed when trying OpenConversation because: {0}", ex.ToString());
                logger.Error(strError);
                msg = ManagerResource.COMMON_ERROR_EXTERNALSERVICE_TIMEOUT;
            }
        ...
        public async Task<ActionResult> GetCurrentConversation()
        {
            ...
            try
            {
                ...
                filterModel.OwnerId = (objectList[0].ObjectType == ((int)EnumMessengerObjectType.Agency) ? objectList[0].ObjectRootId : objectList[1].ObjectRootId);
                filterModel.UserTwo = (objectList[0].ObjectType == ((int)EnumMessengerObjectType.Admin) ? objectList[0].ObjectRootId : objectList[1].ObjectRootId);
        ...
        public async Task<ActionResult> Send(MessageSendingModel model)
        {
            ...
            try
            {
                ...
                if (conversationId == 0)
                {
                    ...
                    record.UserObjectId = (objectList[0].ObjectType == ((int)EnumMessengerObjectType.Agency) ? objectList[0].ObjectId : objectList[1].ObjectId);

                    record.TargetObjectType = (int)EnumMessengerObjectType.Admin;
                    record.TargetObjectId = (objectList[0].ObjectType == ((int)EnumMessengerObjectType.Admin) ? objectList[0].ObjectId : objectList[1].ObjectId);
                ...
                record.UserObjectType = (int)EnumMessengerObjectType.Agency;
        ...
        public async Task<ActionResult> NewMessage(IdentityConversationReply msgItem)
        {
            ...
            try
            {
                ...
                if(newMsg != null)
                {
                    if(newMsg.UserObjectType == (int)EnumMessengerObjectType.Agency)
                    {
                        senderInfo = GetByStaffId(msgItem.UserId);
                    }
                    else if(newMsg.UserObjectType == (int)EnumMessengerObjectType.Admin)
                    {
                        var targetUserResult = AdminServices.GetOperatorProfileAsync(new ApiCommonOperatorInfoModel { Id = newMsg.UserId }).Result;
                        if (targetUserResult != null && targetUserResult.value != null)
                        {
                    ...
                    if (newMsg.TargetId == (int)EnumMessengerObjectType.Agency)
                    {
                        receiverInfo = GetByStaffId(msgItem.UserId);
                    }
                    else if (newMsg.TargetObjectType == (int)EnumMessengerObjectType.Admin)
                    {
                        var targetUserResult = AdminServices.GetOperatorProfileAsync(new ApiCommonOperatorInfoModel { Id = newMsg.TargetId }).Result;
                        if (targetUserResult != null && targetUserResult.value != null)
                        {
                            receiverInfo = JsonConvert.DeserializeObject<IdentityUser>(targetUserResult.value.ToString());
                        }
                ...
                if (currentUserId == msgItem.UserId && msgItem.UserObjectType == (int)EnumMessengerObjectType.Agency)
                {
        ...
        private List<IdentityConversationObject> InitConversation(int receiverId)
        {
            ...
            try
            {
                ...
                from.ObjectType = (int)EnumMessengerObjectType.Agency;

                var to = new IdentityConversationObject();
                to.ObjectRootId = receiverId;
                to.ObjectType = (int)EnumMessengerObjectType.Admin;
