    ...
    public class PermissionHelper
    {
        ...
        public static bool CheckPermission(string actionName = "", string controllerName = "", string currentUserId = "")
        {
            ...
            try
            {
                ...
                {
                    var userInfo = AccountHelper.GetUserById(currentUser);
                    if (userInfo != null)
                    {
        ...
        public static bool CheckIsPic(int picId)
        {
            var hasPic = false;
            var currentUser = string.Empty;
            try
            {
                ...
                if (!string.IsNullOrEmpty(currentUser))
                {
                    ...
                    if (userInfo != null)
                    {
                        if (userInfo.StaffId == picId || userInfo.ParentId == 0)
                        {
                            return true;
                        }
                    }
            ...
            {
                var strError = string.Format("Could not hasPic of user [{0}] because: {1}", currentUser, ex.ToString());
                logger.Error(strError);
            }
        ...
        public static List<IdentityPermission> GetAllPermissionDirectly(string currentUserId)
        {
            ...
            try
            {
                ...
                if (perList == null)
                {
                    ...
                    perList = _rolesIdentityStore.GetPermissionsByUser(currentUserId);
                    if (perList != null && perList.Count > 0)
                    {
                        //Write to cache
                        cacheProvider.Set(myKey, perList);
