...
namespace Manager.WebApp.Helpers
{
    public class MenuHelper
    {
        ...
        public static IEnumerable<IdentityMenu> GetAdminNavigationMenuItems(string userId)
        {
            var cacheKeyByUser = string.Format(CST_ALL_MENU_ITEMS_PATTERN, userId);
            var cacheProvider = GlobalContainer.IocContainer.Resolve<ICacheProvider>();

            IEnumerable<IdentityMenu> result;
            if (!cacheProvider.Get(cacheKeyByUser, out result))
            {
                var menuItems = GetMenuItemsFromDatabase(userId);
                if (menuItems != null && menuItems.Count > 0)
                {
                    result = menuItems.OrderBy(m => m.SortOrder);
                    cacheProvider.Set(cacheKeyByUser, result);
        ...
        public static IdentityMenu GetFirstMenuItem(string userId)
        {
            var cacheKeyByUser = string.Format(CST_ALL_MENU_ITEMS_PATTERN, userId);
            var cacheProvider = GlobalContainer.IocContainer.Resolve<ICacheProvider>();
            ...
            var firstItem = new IdentityMenu();
            if (result != null && result.Count() > 0)
            {
                foreach (var item in result)
                {
                    if (item.SubMenu.HasData())
                    {
                        ...
                        {
                            if (subMenu.Action == "Index" && PermissionHelper.CheckPermission(subMenu.Action, subMenu.Controller, userId))
                            {
        ...
        public static bool CheckCurrentActionCustom(IdentityMenu mi, bool ignoreAction = false)
        {
            if (HttpContext.Current == null)
                return false;
            ...
            if (!string.IsNullOrEmpty(HttpContext.Current.Request["act_active"]))
            {
                actionName = HttpContext.Current.Request["act_active"].ToString();
                controllerName = HttpContext.Current.Request["ctl_active"].ToString();
            }
            else
            {
                var routeValues = HttpContext.Current.Request.RequestContext.RouteData.Values;
                ...
                if (actionName.ToLower() == "edit" || actionName.ToLower() == "detail" || actionName.ToLower() == "create" || actionName.ToLower() == "update")
                {
                    actionName = "Index";
                }
            }

            return CheckCurrentActionCustom(mi, actionName, controllerName, ignoreAction);
        }

        public static bool CheckCurrentGroupCustom(IdentityMenu gmi, bool igoreAction = false)
        {
            var result = false;
            if (CheckCurrentActionCustom(gmi, igoreAction))
            {
                result = true;
            }
            ...
            if (gmi.SubMenu != null && gmi.SubMenu.Count > 0)
            {
                ...
                if (result == false)
                {
                    foreach (var smi in gmi.SubMenu)
                    {
                        if (CheckCurrentGroupCustom(smi, true))
                        {
                            result = true;
                        }
                    }
