...
{
    public class A_NotificationServices
    {
        private static readonly ILog logger = LogProvider.For<A_NotificationServices>();

        public static async Task<ApiResponseCommonModel> GetUnreadNotifCountAsync(ApiAgencyStaffModel model)
        {
            var currentAction = "GetUnreadNotifCountAsync";
            var strError = string.Empty;
            var result = new ApiResponseCommonModel();

            var _baseUrl = string.Format("{0}/api/A_notifications/unread_count", SystemSettings.MainApi);
        ...
        public static async Task<ApiResponseCommonModel> GetNotificationsAsync(ApiAgencyGetListNotificationModel model)
        {
            var currentAction = "GetNotificationsAsync";
            var strError = string.Empty;
            var result = new ApiResponseCommonModel();

            var _baseUrl = string.Format("{0}/api/A_notifications/get_list", SystemSettings.MainApi);
        ...
        public static async Task<ApiResponseCommonModel> MarkIsReadAsync(ApiMarkIsReadAgencyNotificationModel model)
        {
            var currentAction = "MarkIsReadAsync";
            var strError = string.Empty;
            var result = new ApiResponseCommonModel();
            var _baseUrl = string.Format("{0}/{1}", SystemSettings.MainApi, "api/A_notifications/mark_is_read");
