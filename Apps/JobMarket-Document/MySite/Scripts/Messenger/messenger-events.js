...
function CreateChatBox(id, html) {
    ...
    $(document).on('keypress', ".msg2send", function (e) {
        ...
        if (e.which == 13) {
            ...
            e.preventDefault();
            typingSent = false;
            messengerHub.server.messageTypingDone($("#CurrentUser").val(), targetId);
        } else {
            if (target.html() != '') {
                if (typingSent == false) {
                    messengerHub.server.userIsTyping($("#CurrentUser").val(), targetId);
                    typingSent = true;
                }
            }
    ...
    window.emojiPicker = new EmojiPicker({
        ...
        popupButtonClasses: 'icon icon-emoji'
    });
