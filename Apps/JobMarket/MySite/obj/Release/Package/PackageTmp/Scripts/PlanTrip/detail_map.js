...
function initMaps(containerId, json, zoom = 14) {
...
function showTripPost(data, el) {
    if (data) {
        ...
        setTimeout(function () { $(el).show(); }, 1000);
    }
}
...
function generateInfobox(data) {
    ...
    var boxId = data.id + "_" + number;
    //var html = '<div class="maps-infobox plantrip"><div class="checkbox add-sign"><input type="checkbox" class="select-func"  id="add_' + data.id + '" data-type="add" data-box="' + boxId + '" data-id="' + data.id + '" /><label for="add_' + data.id + '"> ' + LanguageDic['LB_ADD_SIGN_PLAN'] + '</label></div><div class="checkbox start-sign"><input type="checkbox" class="select-func"  id="start_' + data.id + '" data-type="start" data-id="' + data.id + '" /><label for="start_' + data.id + '">' + LanguageDic['LB_START_PLAN'] + '</label></div><div class="checkbox remove-sign-dln"><input type="checkbox" class="select-func"  id="remove_' + data.id + '" data-type="remove" data-id="' + data.id + '" /><label for="remove_' + data.id + '">' + LanguageDic['LB_REMOVE_SIGN_PLAN'] + '</label></div></div >';
    var html = '<div class="maps-infobox plantrip"><div class="checkbox add-sign"><input type="checkbox" class="select-func"  id="add_' + data.randomid + '" data-type="add" data-place="' + data.id + '" data-id="' + data.randomid + '" /><label for="add_' + data.randomid + '"> ' + LanguageDic['LB_ADD_SIGN_PLAN'] + '</label></div><div class="checkbox start-sign"><input type="checkbox" class="select-func"  id="start_' + data.randomid + '" data-type="start" data-id="' + data.randomid + '" /><label for="start_' + data.randomid + '">' + LanguageDic['LB_START_PLAN'] + '</label></div><div class="checkbox remove-sign-dln"><input type="checkbox" class="select-func"  id="remove_' + data.randomid + '" data-type="remove" data-id="' + data.randomid + '" /><label for="remove_' + data.randomid + '">' + LanguageDic['LB_REMOVE_SIGN_PLAN'] + '</label></div></div >';
...
function showInfoPlan(array, el) {
    ...
    for (var i = 0; i < array.length; i++) {
        ...
        var marker = getMarkerById(array[i].data.randomid);
        var styleTxt = '';
        if (data.img)
            styleTxt = 'background: url(' + data.img + ') no-repeat center center;background-size: contain;';
        marker.setContent('<div style="' + styleTxt + '" class="maps-marker ' + marker.data.category + '"  data-place="' + marker.data.id + '" data-groupcode="' + data.group_code + '"><i>' + (i + 1) + '</i><p><span>' + marker.data.title + '</span><p></div>');
    }
    $(el).html(html);
...
function restorePlantrip(json, makerOnly = false) {
