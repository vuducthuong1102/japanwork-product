...
namespace MySite.Services
{
    public class MyCloudServices
    {
        ...
        public static async Task<ResponseApiModel> CheckOnlineAsync(CheckUserOnlineModel model)
        {
            var strError = string.Empty;
            var result = new ResponseApiModel();
            var _baseUrl = string.Format("{0}/{1}", MyCloudSettings.MyCloudServer, "api/user/checklistonline");
            ...
            {
                strError = string.Format("Failed when calling api CheckOnlineAsync - {0} because: {1}", _baseUrl, ex.ToString());
