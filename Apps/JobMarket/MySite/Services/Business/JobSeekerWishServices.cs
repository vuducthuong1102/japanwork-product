...
{
    public class JobSeekerWishServices
    {
        private static readonly ILog logger = LogProvider.For<JobSeekerWishServices>();
        ...
        public static async Task<ApiResponseCommonModel> GetDetailAsync(int id)
        {
            ...
            var _baseUrl = string.Format("{0}/api/job_seeker_wish/{1}", SystemSettings.MySiteApi, id);
        ...
        public static async Task<ApiResponseCommonModel> UpdateAsync(ApiJobSeekerWishModel model)
        {
            ...
            var strError = string.Empty;
            var result = new ApiResponseCommonModel();
            var _baseUrl = string.Format("{0}/{1}", SystemSettings.MySiteApi, "api/job_seeker_wish");
