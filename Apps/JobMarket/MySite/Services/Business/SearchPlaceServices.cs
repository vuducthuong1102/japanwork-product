...
{
    public class SearchPlaceServices
    {
        private static readonly ILog logger = LogProvider.For<SearchPlaceServices>();
        ...
        public static async Task<ApiResponseCommonModel> GetSuggestionsByPagingAsync(ApiGetListByPageModel model)
        {
            ...
            var _baseUrl = string.Format("{0}/{1}?keyword={2}&page_index={3}&page_size={4}", SystemSettings.MySiteApi, "api/regions/get_suggestions", model.keyword, model.page_index, model.page_size);
